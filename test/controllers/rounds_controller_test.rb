...
class RoundsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "distances shell renders an offline-capable standalone page" do
    ...
    assert_match(/\bpublic\b/, response.headers["Cache-Control"].to_s)
  end
  ...
  test "native round tracker links to the distances shell and keeps an offline overlay" do
    unlock_course_round!(@course)
    headers = { "HTTP_USER_AGENT" => "Grind/1.0 Hotwire Native iOS; Turbo Native iOS;" }

    get round_course_path(@course), headers: headers
    assert_response :success
    assert_select "a[href=?][data-action=?]", distances_path, "click->round#openDistances"
    assert_select "[data-round-target=distancesPanel]", count: 1
  end
