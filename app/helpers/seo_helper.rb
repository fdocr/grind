...
module SeoHelper
  ...
  DEFAULT_DESCRIPTION = "Grind keeps your score, gives you GPS distances and tracks simple stats that show where you're dropping shots on the course."
  DEFAULT_IMAGE_PATH = "/og.png"

  def seo_page_title
    page_title = content_for(:seo_title).presence || content_for(:title).presence
    page_title.present? ? "#{page_title}" : SITE_NAME
  end
